...
-(void)updateDBtable:(NotePage *)notePage{
    ...
    if([db open]){
        NSString *sqlUpdeteTable = [NSString stringWithFormat:@"UPDATE %@ SET '%@' = '%@' , '%@' = '%@' , '%@' = '%@' WHERE %@ = %d",TABLENAME,TITLE,notePage.titile,CONTENT,notePage.content,TIME,notePage.time,ID,notePage.noteID];
...
-(BOOL)deleteDBtableList:(NotePage *)notePage{
    if([db open]){
        ...
        NSString *sqlDelete = [NSString stringWithFormat:@"delete from %@ where %@ = %d",TABLENAME,ID,notePage.noteID];
